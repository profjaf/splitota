...
filePath = [pwd,'\input\'];
Res2 = load([filePath,'plateVib2.txt']);
dt = 1/fs;
Np = length(Res2);
...
%             title('Differnce actual - real(Reconstructed)', 'FontSize', 11, 'Color', 'k')
            ylabel({'Diff. (m/s^2)'});xlabel('time (s)')

            figure(9)
            bsize=2;
            endplot=fix(Nend/bsize);
            bp=fix(2000/bsize);
            plot(blktime(Nst:bsize:Nend),abs(FRFOrder(Nst:bsize:Nend,1)),'-bs','LineWidth',1,'MarkerEdgeColor','b', 'MarkerSize',4,'MarkerIndices',1:bp:endplot);ylim([0 max(1.65*abs(FRFOrder(Nst:bsize:Nend,2)))]);
            hold on
