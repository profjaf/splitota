function [x,bw] = vkNeilOneOrd(y,f,fs,r,filtord)
%
...
elseif filtord == 1
 ...
 bw = (fs/2)*(0.0652097315./r).^0.5;
 T = 2.85*r.^0.5;
elseif filtord == 2
 NR = N-3;
 e = ones(NR,1);
 A = spdiags([e -3*e 3*e -e],0:3,NR,N);
 bw = (fs/2)*(0.020756902./r).^(1/3);
 T =  2.80*r.^(1/3);
else
 error('filtord must be element of [0,1,2].');
end
AA = r*r*A'*A + speye(N);%speye much faster because matlab will omit zero calculations
jay=sqrt(-1);
